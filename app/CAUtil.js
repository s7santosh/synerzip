...
const adminUserPasswd = process.env.ADMINPSWD;
...
var enrollAdmin = async (caClient, wallet, orgMspId) => {
	try {
...
exports.registerAndEnrollUser = async (caClient, wallet, orgMspId, userId, affiliation, attributes) => {
	try {
		...
		var response= {
			success: true,
			message: 'user registered fuccessfully'
		}
		...
		var response= {
			success: false,
			message: 'user not registered fuccessfully'
		}
		return response;
	}
};
